...
function RegisterPage() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!userDoc.exists()) {
        // Create new seller user document
        await setDoc(doc(db, "users", user.uid), {
          accountId: user.uid,
          accountName: user.displayName || "Seller",
          role: "seller",
        ...
        // Create shop document with isApproved: false
        await addDoc(collection(db, "shops"), {
          sellerId: user.uid,
          shopName: "",
          shopCity: "",
        ...
        navigate("/login", { state: { message: t("shopApplicationPending") } });
      } else {
